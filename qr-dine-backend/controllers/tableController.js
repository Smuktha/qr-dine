...
const FRONTEND_URL =
  process.env.FRONTEND_URL || "https://qr-dine-eight.vercel.app";
...
export const addTable = asyncHandler(async (req, res) => {
  try {
    ...
    // Generate QR code
    const qrURL = `${FRONTEND_URL}/?tableId=${newTable._id}`;
...
export const updateTable = asyncHandler(async (req, res) => {
  try {
    ...
    if (tableNumber) {
      ...
      // Regenerate QR
      const qrURL = `${FRONTEND_URL}/?tableId=${table._id}`;
      const qrCodeData = await QRCode.toDataURL(qrURL);
...
export const generateAllQRCodes = asyncHandler(async (req, res) => {
  try {
    ...
    res.json({
      message: `QR codes generated for ${tables.length} tables`,
      tables,
    });
  } catch (error) {
    res
      .status(500)
      .json({ message: "Failed to generate QR codes", error: error.message });
  }
});